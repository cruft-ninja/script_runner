...
3\. Ensure the bash scripts referenced in `runner.py` exist in the specified paths (e.g., `/home/ninja/apps/script_runner/bash/`). Customize paths as needed.
...
Customize the `scripts` list in `runner.py` to add/remove scripts:

\`\`\`python

scripts = \[

    ("Button Label", needs_sudo: bool, "/path/to/script.sh", \["tag1", "tag2"\], "Tooltip description"),
...
\- Adjust `BUTTONS_PER_ROW`, `GRID_PAD`, and `LOG_LINES` for layout preferences.
